# Setup Telegram Bot Configuration for Group
# This script adds bot token and group chat ID to .env.local

$botToken = "7657983245:AAEx45-05EZOKANiaEnJV9M4V1zeKqaSgBM"
$groupChatId = "-4913116305"
$envFile = ".env.local"

Write-Host "Setting up Telegram Bot Configuration for Group..." -ForegroundColor Cyan
Write-Host ""
...
if (Test-Path $envFile) {
    ...
    if ($content -match "TELEGRAM_CHAT_ID") {
        ...
        Write-Host "Updating with group chat ID..." -ForegroundColor Yellow
        $content = $content -replace "TELEGRAM_CHAT_ID=.*", "TELEGRAM_CHAT_ID=$groupChatId"
    } else {
        Write-Host "Adding TELEGRAM_CHAT_ID to .env.local" -ForegroundColor Green
        $content += "TELEGRAM_CHAT_ID=$groupChatId`n"
    }
    ...
    Write-Host ""
    Write-Host "Telegram configuration updated for group!" -ForegroundColor Green
    Write-Host ""
    Write-Host "Configuration:" -ForegroundColor Cyan
    Write-Host "  Bot Token: $botToken" -ForegroundColor White
    Write-Host "  Group Chat ID: $groupChatId" -ForegroundColor White
    Write-Host "  Group Name: FIFTO TEST Group" -ForegroundColor White
    Write-Host ""
    Write-Host "Next steps:" -ForegroundColor Cyan
    Write-Host "  1. Make sure your bot (@manififtobot) is added to the group" -ForegroundColor White
    Write-Host "  2. Run: npm run test:telegram" -ForegroundColor White
    Write-Host "  3. Restart your server" -ForegroundColor White
    Write-Host "  4. Run a test login to receive notifications in the group" -ForegroundColor White
} else {
    ...
    Write-Host "  2. Make sure your bot (@manififtobot) is added to the group" -ForegroundColor White
